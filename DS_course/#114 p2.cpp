	...
	BinarySearchTree *delete_node(int target, BinarySearchTree *node){// return updated tree
        ...
        if(target<node->data)
            ...
            if(!node->left&&!node->right){
               ...
               BinarySearchTree *child=node->right, *parent=node;
                // get successor
                while(child->left)
...
//                node->right=delete_node(node->data, node->right);
                // we can easily do it in recursion, but we now need to update the right node iterative
                // first know that the node i'm search is either 1 child, or 0 child
